    ...
    changeStatus(e) {
        ...
        for (var key in orders) {
            if (orders[key].key === orderPostKey) {
                orders[key].order.status = 'delivered'
        ...
        firebase.database().ref(`order/forRestaurant/${uid}`).on('value', (e) => {
            ...
            for (var key in order) {
                for (var key2 in order[key]) {
                    if (order[key][key2].status === 'inprogress') {
                        orders.push(
            ...
            <div>
                ...
                <ul style={{listStyleType: 'none'}}>
                    {orders.length && orders.map((e) => {
                        return (<li style={{margin: '5px auto' , width: 500, height: 50,borderRadius: 50, border: '2px dashed white'}} key={e.key}>{<div style={{display: 'flex', justifyContent:'space-around'}}><h3>{e.order.productName}</h3><h3>Status: {e.order.status}</h3><button style={{height: 40, marginTop: 5,border: 'none', background: 'none', borderRadius: 50, boxShadow: 'inset 0px 0px 2px green'}} onClick={(e) => { this.changeStatus(e) }} id={e.order.userId} className={e.key}>Click to Deliverd</button></div>}</li>)                        
                    })}
                </ul>
            </div>
        )
